...
des portes, la propagation des signaux et le parcours du circuit."""
...
def gate_8not(inputs: list[int]) -> list[int]:
    ...
    Returns:
        Valeur 8 bits inversée bit à bit.
    """
...
def gate_8maker(inputs: list[int]) -> list[int]:
    ...
    Returns:
        Liste à un seul élément contenant la représentation entière.
    """
...
def gate_8breaker(inputs: list[int]) -> list[int]:
    ...
    Returns:
        Représentation binaire 8 bits sous forme de liste d'entiers.
    """
...
def gate_delay(inputs: list[int], gate: Gate) -> list[int]:
    ...
    Returns:
        Bit tel qu'il était il y a un tick.
    """
    old = gate.old_output
...
def gate_8subtractor(inputs: list[int]) -> int:
    """Soustracteur 8 bits sans retenue empruntée.
...
def gate_8mux(inputs: list[int]) -> list[int]:
    ...
    Args:
        inputs: Une liste où l'index 0 est le sélecteur (0-7) 
...
def gate_register(inputs: list[int], gate: Gate) -> list[int]:
    ...
    Args:
        inputs: Valeur d'entrée (8), Sauvegarder (Save), Charger (Load)
...
class Engine:
    ...
    def sort_gates(self, chip: Chip) -> tuple[list[str], list[str], list[str]]:
        ...
        Returns:
            Tuple contenant les listes d'IDs des portes logiques, des entrées et des sorties.
        """
        gates: list[str] = []
        inputs: list[str] = []
        outputs: list[str] = []
    ...
    def reset_input_validation(
        ...
        """Réinitialise l'état de validation pour toutes les portes et tous les chemins.
    ...
    def get_wired_inputs_map(self, connections: dict[str, list]) -> dict[str, set[int]]:
        """Associe les ports d'entrée des portes à leur état de câblage.
    ...
    def run_propagation_loop(
        ...
        if iterations >= safeguard_max:
            logger.warning("Sauvegarde atteinte. Boucle infinie ou trop complexe.")

    @profile
    def propagate_values(self, chip: Chip, visible=True) -> None:
        """Initialise et exécute la séquence de propagation du signal.
